...
在简历配置页点击“管理字段”，可按分区新增、编辑或删除内置及自定义字段，支持单行、多行、日期、下拉、邮箱、电话和网址。每份简历独立配置，同一分区的多条经历共用字段定义。修改后点击“应用此字段”“完成”，再保存简历；取消会放弃本次字段管理的修改。删除会清除该分区全部记录的对应内容；类型或选项变更导致内容不兼容时，会先提示确认。字段配置随模板复制、备份和同步保留，AI 预填与自动填表均使用当前配置。
...
接收数据后，如果浏览器同步存储中的有效版本已经包含本机全部记录，就不再重复上传整份副本。已有的其他设备副本与删除记录仍保留，避免丢失并发修改或让已删除的简历重新出现。

#### 同步容量不足时如何处理

1. 在“简历配置 → 简历管理 → 导出全部”保存 `简历模板备份-日期.json`。导出包含全部简历的结构化字段与原文，先保存当前编辑；它不会释放同步空间。
2. 更新扩展代码后，在 `chrome://extensions`（Edge 使用 `edge://extensions`）重新加载扩展，再重新打开简历配置页，点击“重试同步”。无需卸载扩展。
3. 查看“同步详情与修复”：失败时也重新读取当前占用，另外显示本次版本大小、更新峰值、更新完成后的预计占用、存储项数量和最大单项大小。读取占用失败时明确显示无法读取，不再沿用上次成功的数值。报错会区分总容量、单项大小及数量上限。容量超限后停止定时与存储变化触发的自动重试；修改简历或点击“重试同步”后再尝试。
4. 总量或更新峰值超限时，备份后删除不需要的简历、冲突副本，或缩短“AI 预填 → 原始简历文本”并保存，再重试。同步内容已经压缩，减少文本后的实际空间变化取决于内容；其他设备旧副本仍可能占用容量。
5. 若完整资料仍然放不下，可关闭简历同步，继续在本机编辑与填表，使用 JSON 导出/导入跨设备迁移。导入全部会覆盖目标设备全部简历，先备份目标设备。关闭同步不会清空已有同步数据。

本地简历位于当前浏览器配置文件的 `chrome.storage.local`，不是项目目录中的 JSON 文件：`resumeTemplates` 保存简历字段与 `rawText` 原文，`activeResumeTemplateId` 保存当前选择，`resumeSyncState` 保存同步版本与待同步状态，`resumeSyncStatus` 保存同步诊断信息。模型配置与 API Key 不在简历导出文件中。

可从 `chrome://version`（Edge 使用 `edge://version`）查看当前“个人资料路径”，对应扩展数据库通常在其 `Local Extension Settings/<扩展 ID>/` 下；扩展 ID 可在扩展管理页查看。请使用“导出全部”生成可恢复备份，不直接修改数据库文件，也不要通过卸载扩展或清空本地存储来处理同步容量错误。

#### 删除后又恢复、更新一直超限时：备份并重建同步

普通同步会先合并远端版本再发布；手动删除分片可能留下不完整清单，旧设备也可能补发副本。若最终数据能装下，但新旧版本同时存放超过 100 KB，可通过以下入口重建，不需要在控制台执行删除脚本：

1. 选定简历最完整的电脑作为来源。其他设备先导出各自简历、关闭扩展内的简历同步，并更新扩展代码。旧版扩展不识别重建标记，不能在重建期间继续同步。
2. 来源电脑重新加载扩展，打开“简历管理 → 同步详情与修复 → 备份并重建同步”。确认以本机为准；只在其他设备上的内容不会自动合并。
3. 程序先保存当前表单、检查重建后容量、在本机保存恢复备份并触发 JSON 下载，然后清理本扩展的旧简历同步键并发布本机数据。不会清空 `storage.local`、模型配置、API Key 或无关同步设置。容量仍超限或本机备份失败时，不清理远端。
4. 重建中断时同步保持暂停，重新打开页面点击“继续重建同步”。任务和备份保存在本机，后台重启后仍可继续；不要在多台电脑同时重建。
5. 其他已更新设备检测到新的重建标记后暂停旧数据上传。重新打开页面或点击“重试同步”，选择“备份本机并接收重建版本”；这会先备份该设备，再用重建后的数据替换本机简历。

恢复备份的 `templates` 可通过“导入全部”恢复来源电脑当时的简历；`recovery` 还保留当时的同步记录和原始远端数据，供诊断使用，不会在普通导入时自动还原到云端。本机另保存 `resumeSyncRecoveryBackup`（重建前）和 `resumeSyncAdoptionBackup`（接收前）。下载被阻止或失败时，请保留这些本机备份。

重建保留本机已知的冲突与删除记录，解决旧副本占位和临时空间阻塞，不会突破浏览器的 100 KB 总容量限制。大版本后续更新仍可能需要再次重建或减少内容。
